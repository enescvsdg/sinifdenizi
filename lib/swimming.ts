import { speciesDefinitions } from "./species.ts";

export type SwimStyle =
  "cruise" | "dart" | "glide" | "hover" | "pulse" | "bottom";
/** Drawn size relative to the aquarium's fish size, by catalogue group. */
const sizes = { small: 0.88, special: 1, large: 1.3 };
export type Swimmer = {
  x: number;
  y: number;
  vx: number;
...
export function swimProfile(species: number) {
  ...
  const size = sizes[speciesDefinitions[species]?.category ?? "special"];
  return {
    style,
    size,
    speed: speeds[style],
    minY: style === "bottom" ? 0.61 : 0.16,
    maxY: style === "bottom" ? 0.81 : 0.73,
  };
}
...
export function createSwimmers(
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let sample = 0; sample < 40; sample++) {
      const candidate = {
        x: 0.09 + random() * 0.82,
        y: profile.minY + random() * (profile.maxY - profile.minY),
      };
      const nearest = fish.length
        ? Math.min(
            ...fish.map((f) =>
              Math.hypot((f.x - candidate.x) * 1.5, f.y - candidate.y),
            ),
          )
        : 1;
      if (nearest > distance) {
    ...
    fish.push({
      ...best,
      vx: direction * profile.speed * (0.8 + random() * 0.3),
      vy: 0,
      depth: i % 3,
      phase: i * 2.7,
      species: kind,
...
export function stepSwimmers(
  ...
  const profiles = fish.map((f) => swimProfile(f.species));
  const forces = fish.map((a, index) => {
    const profile = profiles[index];
    const pulse = Math.sin(
      time * (profile.style === "dart" ? 1.8 : 0.55) + a.phase,
    );
    const targetSpeed =
      profile.speed * (1 + pulse * (profile.style === "dart" ? 0.3 : 0.12));
    let ax = (a.direction * targetSpeed - a.vx) * 0.42;
    let ay =
      Math.sin(time * 0.48 + a.phase) *
      (profile.style === "pulse" ? 0.011 : 0.0045);
    if (profile.style === "pulse")
      ay -= Math.max(0, Math.sin(time * 2 + a.phase)) * 0.003;
    for (let j = 0; j < fish.length; j++) {
      if (j === index) continue;
      const b = fish[j];
      const dx = (a.x - b.x) * safeAspect,
        dy = a.y - b.y;
      const distance = Math.hypot(dx, dy);
      // Bigger creatures keep proportionally more room around them.
      const spacing =
        ((a.depth === b.depth ? 0.17 : 0.115) *
          (profile.size + profiles[j].size)) /
        2;
      if (distance < spacing) {
  ...
  fish.forEach((a, i) => {
    ...
    a.vy = Math.max(
      -0.024,
      Math.min(0.024, (a.vy + forces[i].ay * delta) * Math.exp(-delta * 0.6)),
    );
