...
test("special creatures unlock at their exact student XP thresholds", () => {
  for (const [id, xp] of [[27, 500], [28, 1500], [29, 3000]]) {
    ...
    if (speciesDefinitions[id].available) assert.equal(changed.students[0].fish, id);
...
test("a fresh classroom uses diverse unlocked creatures", () => {
  const state = initialState();
  const available = speciesDefinitions.filter((fish) => canChooseSpecies(fish.id, 0)).length;
  assert.equal(new Set(state.students.map((student) => student.fish)).size, Math.min(available, state.students.length));
  assert.ok(state.students.every((student) => canChooseSpecies(student.fish, student.xp)));
});
